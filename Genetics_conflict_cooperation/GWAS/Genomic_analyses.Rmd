...
library("magick") ## image_read function
library("wesanderson") ## color palettes
library("scales") ## add semi-transparent background in legend
library("car") ## Anova function
library("emmeans") ## post hoc tests
library("PopGenome") ## calculate nucleotide diversity
library("readxl") ## read_excel function
library("gridGraphics") ## base r plots
```
...
## add on the shared y axis title
y_axis <- ggdraw() + draw_label("Allelic effect size for Nodule weight", size = 16, angle=90)

## put them together
es1 <- plot_grid(y_axis, es, ncol=2, rel_widths=c(0.1, 1)) 

## create plot titles
t1 <- ggdraw() + draw_label("Phenotypic", size = 20, hjust = 0.3, fontface = "bold")
...
## add on the shared y axis title
y_axis <- ggdraw() + draw_label("Rhizobium relative fitness", size = 16, angle=90)
...
## add on the shared y axis title
y_axis <- ggdraw() + draw_label("Allelic effect size for Rhizobium relative fitness", 
                                size = 16, angle=90)
...
## add on the shared y axis title
y_axis <- ggdraw() + draw_label("Allelic effect size for Shoot biomass", size = 16, angle=90)
...
gc <- plot_grid(gc_ra[["shoot_vs_fit_DZA"]],
                       gc_ra[["shoot_vs_fit_A17"]] +
                         xlab("Rhizobium relative fitness"),
          ncol = 1,
          nrow = 2,
          align = "v",
          labels = NULL)
...
save_plot("./Figures/sym_shoot.pdf", p_comb.shoot,
          ncol = 1.5, # we're saving a grid plot of 2 columns
          nrow = 4, # and 3 rows
          # each individual subplot should have an aspect ratio of 1.3
          base_aspect_ratio = 1.3
...
save_plot("./Figures/sym_shoot.png", p_comb.shoot,
          ncol = 1.5, # we're saving a grid plot of 2 columns
          nrow = 4, # and 3 rows
          # each individual subplot should have an aspect ratio of 1.3
          base_aspect_ratio = 1.3
...
include_graphics("./Figures/sym_shoot.png")
...
## add on the shared y axis title
y_axis <- ggdraw() + draw_label("Leaf chlorophyll A", size = 16, angle=90)
...
## add on the shared y axis title
y_axis <- ggdraw() + draw_label("Allelic effect size for Leaf chlorophyll A", size = 16, angle=90)
...
save_plot("./Figures/sym_chloro.pdf", p_comb.chloro,
          ncol = 1.5, # we're saving a grid plot of 2 columns
          nrow = 4, # and 3 rows
          # each individual subplot should have an aspect ratio of 1.3
          base_aspect_ratio = 1.3
...
save_plot("./Figures/sym_chloro.png", p_comb.chloro,
          ncol = 1.5, # we're saving a grid plot of 2 columns
          nrow = 4, # and 3 rows
          # each individual subplot should have an aspect ratio of 1.3
          base_aspect_ratio = 1.3
...
include_graphics("./Figures/sym_chloro.png")
...
## summarize all genes first
vars <- c("Pi","Tajima.D","Fu.Li.F","Fu.Li.D") 
all_genes.sum <- all_genes %>% 
  summarise_at(vars, mean, na.rm = TRUE) %>%
  gather(stat, mean, Pi:Fu.Li.D)
all_genes.sum$mean.r <- round(all_genes.sum$mean, 3)
all_genes.sum$mean_sig <- paste0(all_genes.sum$mean.r, " (NA)")
all_genes.sum.f <- all_genes.sum %>%
  mutate(cat = paste0("All")) %>%
  select(cat, stat, mean_sig)
  

## focal genes:
### format mean and p-vals, keep certain cols
real_data.k <- real_data %>%
  mutate(mean.r = round(mean, 3)) 
real_data.k$p_all <- ifelse(real_data.k$pvals.all > 0.1, "NS", real_data.k$pvals.all)
real_data.k$p_sig <- ifelse(real_data.k$pvals.sig > 0.1, "NS", real_data.k$pvals.sig)
real_data.k$mean_sig <- ifelse(real_data.k$cat == "GWAS" | 
                                 real_data.k$cat == "sig", 
                               paste0(real_data.k$mean.r, 
                                      " (", 
                                      real_data.k$pvals.all,")"), 
                               paste0(real_data.k$mean.r, 
                                      " (", 
                                      real_data.k$pvals.sig,")"))
real_data.f <- real_data.k %>%
  select(cat, stat, mean_sig)

## combine all and focal gene stats
comb <- rbind(all_genes.sum.f, real_data.f)

## spread means for each stat
comb.w <- comb %>%
  spread(stat, mean_sig, fill = NA, convert = FALSE)

## save
write.csv(comb.w, file = "./Data_output/neutrality_stats_simplified.csv",
     row.names = FALSE) ## Updated Supp. Table S1
```