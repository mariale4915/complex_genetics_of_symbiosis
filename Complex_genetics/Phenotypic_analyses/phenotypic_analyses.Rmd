...
```{r controls}
load(file = "./Data/ensifer_dataset.Rdata") ## loads ds_ensifer

ds_control <- ds_ensifer %>%
  select(line_exp, exp, type, shoot, nod) %>%
  group_by(line_exp, exp, type) %>%
  summarise(mean_shoot = mean(shoot, na.rm=T), SE_shoot = sd(shoot, na.rm=T)/sqrt(length(shoot)),
            mean_nod = mean(nod, na.rm=T), SE_nod = sd(nod, na.rm=T)/sqrt(length(nod)))
## replace NaN with 0:
...
write.csv(ds, "./Data/dataset_treat.csv", row.names = FALSE)
save(ds, file = "./Data/ds_treat.Rdata")
...
```{r raw_means}
load(file = "./Data/ds_treat.Rdata") ## loads ds
...
save(ds_raw_means, file = "./Data/raw_means_long.Rdata")
...
save(ds_means.w, file = "./Data/raw_means_wide.Rdata")
```
...
emmeans_out <- mapply(FUN = emmeans_function, traits = traits_list, 
                      exps = exps_list, USE.NAMES = TRUE, MoreArgs = list(df = ds))
...
save(emmeans.w, file = "./Data/emmeans_wide.Rdata")
...
save(emmeans, file = "./Data/emmeans_long.Rdata")

## regress emmeans and raw_means to make sure they were correctly calculated
load("./Data/raw_means_long.Rdata") ## loads ds_raw_means
load(file = "./Data/emmeans_long.Rdata") ## loads emmeans
...
```{r plasticity}
load(file = "./Data/emmeans_wide.Rdata") ## loads emmeans.w
...
save(emmeans.w_Ensifer, file = "./Data/emmeans.w_Ensifer.Rdata") ## will get used to make phenotype file
...
plast_out <- lapply(trait.list, plast_func, df = emmeans.w_Ensifer)

## shoot biomass figure only
fig_base <- plot_grid(plast_out[[4]][[2]], plast_out[[4]][[3]],  
          ncol = 1,
          nrow = 2,
          align = "v",
          labels = NULL)
...
fig.xaxis <- add_sub(fig_base, "Strain", size = 24, hjust = 0.5)

# add on the shared y axis title
fig_y_axis <- ggdraw() + draw_label("Plasticity (log response ratio)", size = 24, angle=90)

# put them together
fig <- plot_grid(fig_y_axis, fig.xaxis, ncol=2, rel_widths=c(0.05, 1)) 
# rel_heights or widths values control text margins

save_plot("./Figures_Tables/strain_plasticity_shoot.png", fig,
          ncol = 1, # we're saving a grid plot of 2 columns
          nrow = 2, # and 3 rows
          # each individual subplot should have an aspect ratio of 1.3
          base_aspect_ratio = 3
...
## all traits figure
fig_base <- plot_grid(plast_out[[4]][[2]] + ggtitle("Shoot biomass"), 
                       plast_out[[1]][[2]] + ggtitle("Chlorophyll"),
                       plast_out[[4]][[3]], plast_out[[1]][[3]],
                       plast_out[[2]][[2]] + ggtitle("Leaves"),
                       plast_out[[3]][[2]] + ggtitle("Plant height"),
                      plast_out[[2]][[3]], plast_out[[3]][[3]], 
...
## Models: split by line, assess GxE
...
## run
GxE_out <- mapply(FUN = GxE_func, traits = trait_list, 
                      lines = line_list, USE.NAMES = TRUE, MoreArgs = list(df = ds))
...
racks_out <- rbind(rack_out.DZA[c("lmm","lmm1","lmm2","lmm3"),
                                c("Chisq","Chi Df","Pr(>Chisq)","trait_line")], 
...
emmeans$exp2 <- ifelse(emmeans$exp == 1 | emmeans$exp == 2, "1", "2")

(p <- ggplot(data = emmeans, aes(x=exp2, y=shoot, group=strain_ID)) + 
  geom_line(position = position_dodge(0.3), color = "grey") +
  geom_point(data = emmeans %>% filter(line == "DZA"), 
             size=4, position = position_dodge(0.3), color = "#4F820D", alpha = 0.5) + 
  geom_point(data = emmeans %>% filter(line == "A17"), 
             size=4, position = position_dodge(0.3), color = "#AD208E", alpha = 0.5) +   
...
(p_nod <- ggplot(data = emmeans, aes(x=exp2, y=nod, group=strain_ID)) + 
  geom_line(position = position_dodge(0.3), color = "grey") +
  geom_point(data = emmeans %>% filter(line == "DZA"), 
             size=4, position = position_dodge(0.3), color = "#4F820D", alpha = 0.5) + 
  geom_point(data = emmeans %>% filter(line == "A17"), 
...
save_plot("./Data/Supp_shoot_exp_rxn.png", p,
          ncol = 1, # we're saving a grid plot of 2 columns
          nrow = 1, # and 3 rows
          # each individual subplot should have an aspect ratio of 1.3
          base_aspect_ratio = 1.3
...
## run
rxn_out <- lapply(trait.list, rxn_func, df = emmeans.rn)

# put all plots into one
rxn_comb <- plot_grid(rxn_out[[1]][[1]],rxn_out[[2]][[1]],
                       rxn_out[[3]][[1]],rxn_out[[4]][[1]],
          ncol = 2,
          nrow = 2,
          align = "hv",
          labels = NULL)
...
## run
herit_out <- mapply(FUN = herit_func, traits = trait_list, 
                      exps = exp_list, USE.NAMES = TRUE, MoreArgs = list(df = ds))
...
strain_sig$sig <- ifelse(strain_sig$'Pr(>Chisq)' < 0.001, "***",
                         ifelse(strain_sig$'Pr(>Chisq)' < 0.01, "**",
                                ifelse(strain_sig$'Pr(>Chisq)' < 0.05, "*",
                                       ifelse(strain_sig$'Pr(>Chisq)' < 0.1, ".", " "))))
...
## save
save(herit.f, file = "./Data/heritability.Rdata") ## needed for cockerham's method
```
...
```{r corr_plot}
load(file = "./Data/emmeans.w_Ensifer.Rdata") ## loads emmeans.w_Ensifer

# Plotting function (option 1)

## loop through vars
trait_list <- c("chloro1","height1","leaf1","shoot")

## source function
source("../Source_code/corr_plot_func.R")

## run
plot_out <- lapply(trait_list, corr_plot_func, df = emmeans.w_Ensifer)

# put all plots into one
fig_base <- plot_grid(plot_out[[1]]+
                        xlab("Chlorophyll (exp 1)")+ 
                        ylab("Chlorophyll (exp 2)"), 
                      plot_out[[2]]+
                        xlab("Plant height (exp 1)")+ 
                        ylab("Plant height (exp 2)"), 
                      plot_out[[3]]+ 
                        xlab("Leaves (exp 1)")+ 
                        ylab("Leaves (exp 2)"), 
                     plot_out[[4]]+ 
                        xlab("Shoot biomass (exp 1)")+ 
                        ylab("Shoot biomass (exp 2)"),
          ncol = 2,
          nrow = 2,
          align = "hv",
          labels = NULL)

save_plot("./Figures_Tables/reg_GxE_op1.png", fig_base,
          ncol = 2, # we're saving a grid plot of 2 columns
          nrow = 2, # and 3 rows
          # each individual subplot should have an aspect ratio of 1.3
          base_aspect_ratio = 1.3
...
# plotting function (option 2), plus reg coefficients
...
## Run
reg_out <- mapply(FUN = lin_reg_func, exp1 = trait_exp1, 
                      exp2 = trait_exp2, USE.NAMES = TRUE, MoreArgs = list(df = emmeans.w_Ensifer))

## put all plots into one
fig_base <- plot_grid(reg_out[[15]]+ 
                        ggtitle("A17") +
                        ylab("Chlorophyll")+ 
                        theme(plot.title = element_text(hjust=0.5, size=24, face = "bold", color = "#AD208E"),
                          axis.title.y = element_text(size = 24, angle=90)),
                      reg_out[[3]]+
                        ggtitle("DZA") +
                        theme(plot.title = element_text(hjust=0.5, size=24, 
                                                        face = "bold", color = "#4F820D")), ## chloro
...
save_plot("./Figures_Tables/reg_GxE_op2.png", fig_base,
          ncol = 2, # we're saving a grid plot of 2 columns
          nrow = 4, # and 3 rows
          # each individual subplot should have an aspect ratio of 1.3
          base_aspect_ratio = 1.3
...
## Extract Reg coeffs
reg_coeffs_out1 <- as.data.frame(rbind(reg_out[[2]][2,],reg_out[[5]][2,],reg_out[[8]][2,],
                                  reg_out[[11]][2,],reg_out[[14]][2,],reg_out[[17]][2,],
                                  reg_out[[20]][2,],reg_out[[23]][2,]))


# extract correlation coefficients
reg_coeffs_out2 <- as.data.frame(rbind(reg_out[[1]],reg_out[[4]],reg_out[[7]],
                                  reg_out[[10]],reg_out[[13]],reg_out[[16]],
                                  reg_out[[19]],reg_out[[22]]))
...
# save
save(reg_coeffs_out, file = "./Data/reg_coeff.Rdata") ## needed for cockerham's method
```

## Cockerham's method

```{r cockerham}
load(file = "./Data/heritability.Rdata") # loads herit.f
...
load(file = "./Data/reg_coeff.Rdata") ## loads reg_coeffs_out
...
## Run
gc_out <- mapply(FUN = gen_corr_func, traits1 = traits1.list, 
                      traits2 = traits2.list, cols = cols.list, USE.NAMES = FALSE, 
