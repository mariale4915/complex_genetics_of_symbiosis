...
# Summarize at the ps-level
  SNPs_ann_ps <- SNPs_sig_long %>%
    filter(trait == "shoot") %>%
    droplevels(.) %>%
    group_by(region, rs, gene_name, RefSeq_ID, ncbi_func, 
             start_pos, end_pos, effect) %>% ## summarized to rs-level
    summarize(
      no_effects = n(),
  ...
  SNPs_ann_ps$dir <- ifelse(SNPs_ann_ps$no_effects > 1 &
                              grepl("shoot[-]", SNPs_ann_ps$assocs, 
                                    fixed = FALSE) &
                                      grepl("shoot[+]", SNPs_ann_ps$assocs, 
                                            fixed = FALSE), "dis",
  ...
  SNPs_ann_ps$GxE <- ifelse(SNPs_ann_ps$dir == "dis" &
                                ...
                                ifelse(SNPs_ann_ps$no_effects > 2 &
                                  SNPs_ann_ps$dir == "con" & 
                                    ...
                                    grepl("A17-4", SNPs_ann_ps$assocs, 
                                            fixed = FALSE),
                                   "concordant_A17",SNPs_ann_ps$dir))))))))
                               
  
  write.csv(SNPs_ann_ps, file = "./Data/Var-lvl_summaries/tradeoffs.csv", 
            row.names = FALSE)
...
save(var_sum_out, file = "./Data/Var-lvl_summaries/var_sums.Rdata")
...
SNPs_sig_long$trait <- recode(as.character(SNPs_sig_long$trait), 
                              chloro1 = "chloro",
                              height1 = "height",
                              leaf1 = "leaf")
...
save(SNPs_ann_gene, file = "./Data/Gene-lvl_summaries/SNPs_ann_gene_all.Rdata")
write.csv(SNPs_ann_gene, "./Data/Gene-lvl_summaries/SNPs_ann_gene_all.csv", 
          row.names = FALSE)
...
save(gene_sum_out, file = "./Data/Gene-lvl_summaries/Venn_diagram_nums.Rdata")
...
```{r Venns}
load(file = "./Data/Gene-lvl_summaries/Venn_diagram_nums.Rdata") # loads gene_sum_out
...
```{r manhattan}
# Use manhattan function to create plots for each trait

## traits to iterate through:
trait.list <- c("chloro1","height1","leaf1","shoot",
                "chloro.plast","height.plast","leaf.plast","shoot.plast")

## source function
source('../Source_code/manhattan_func.R')

manhattan_out <- sapply(trait.list, manhattan_func, 
                        simplify = FALSE, USE.NAMES = TRUE)

mans <- plot_grid( manhattan_out[["chloro1"]] + ggtitle("Chlorophyll A"), 
                    manhattan_out[["height1"]] + ggtitle("Plant height"), 
                    manhattan_out[["leaf1"]] + ggtitle("Leaves"), 
                    manhattan_out[["shoot"]] + ggtitle("Shoot biomass"),
                   manhattan_out[["chloro.plast"]], 
                   manhattan_out[["height.plast"]], 
                   manhattan_out[["leaf.plast"]],
                   manhattan_out[["shoot.plast"]],
                    ncol = 4, nrow = 2, align="hv", labels=c("A","","","",
                                                             "B","","",""))

# add on the shared x axis title
fig_x_axis <- add_sub(mans, "Genomic position (Mbp)", size=24)

# add on the shared y axis title
fig_y_axis <- ggdraw() + draw_label("Average absolute \n effect size", size = 24, angle=90)

# put them together
fig <- plot_grid(fig_y_axis, fig_x_axis, ncol=2, rel_widths=c(0.05, 1)) 
# rel_heights or widths values control text margins

save_plot("./Figures_Tables/manhattans_all.png", fig,
          ncol = 4, # we're saving a grid plot of 2 columns
          nrow = 2, # and 2 rows
          # each individual subplot should have an aspect ratio of 1.3
          base_aspect_ratio = 1.3)

include_graphics("./Figures_Tables/manhattans_all.png")
```
